...
The compiler API keeps these purposes separate. On-demand preparation checks the accepted request against the project, source version, run, compiler and version, model, and prompt digest before it records work. Eager preparation accepts only sources captured as `eager`. Replay, evaluation, hindsight, and fixture bootstrap use separate entry points, so a new caller must choose the rule it intends to follow.
