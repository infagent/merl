...
Support-only reviews preserve temporal state; a changed date requires a new
semantic interpretation. Deferral reasons remain spans in protected source
payloads, so erasure removes the prose without deleting temporal provenance.
...
A review request fixes the actor, candidate, action, replacement, and reason reference before evaluation. Policy records a new command input and evaluates current grants. Acceptance and correction commit a semantic event with review-to-assertion lineage; rejection commits a control record that closes the candidate without creating its proposed object. Each accepted review advances the project revision and reaches delta and inbox. Review control records stay out of semantic views and compiler context. Reasons live behind independently erasable payload references.
