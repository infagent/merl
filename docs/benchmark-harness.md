...
Before an evaluator builds v4, maintainers audit the complete public first-release workflow and resolve every gap that does not depend on held-out results. They then freeze the product, corpus, and harness contracts. The evaluator builds and reviews v4 against those contracts, after which #24 can close. The project freezes the exact RC and evaluation configuration before #32 runs v4. The post-evaluation audit may assemble evidence and make the release decision; it may not change the evaluated candidate.
