...
New GitHub captures use `merl.corpus-fixture/v2`. They record the Issue's upstream update time and stable IDs for labels and assignees. The latest captured version of each Issue body or comment also records that entity's `updatedAt` at capture. That timestamp can follow the body edit, so a historical compiler context cannot treat it as known at the edit's cutoff. Earlier versions leave it unknown unless an earlier capture established it. Merl still reads v1 fixtures under their original rules. Missing v1 update times and label IDs remain unknown. Keep frozen v1 files unchanged. A later recapture or augmentation needs its own versioned artifact and provenance.
