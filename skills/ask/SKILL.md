...
Resolve `<plugin-root>` as the directory two levels above this loaded `SKILL.md`, using its absolute path. Decide whether the request blocks current work. For `--non-blocking`, include `--continuation` explaining why the request exists, what can continue independently, and how a successor should integrate the answer. Invoke `<plugin-root>/bin/merl ask ... --json` directly; never probe for `merl` on `PATH` or edit board Markdown directly. If Merl reports a possible duplicate, inspect that request instead of publishing another.
